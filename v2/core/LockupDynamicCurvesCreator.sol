...
contract LockupDynamicCurvesCreator {
    ...
    function createStream_UnlockInStepts() external returns (uint256 streamId) {
        // Declare the total amount as 100 DAI
        uint128 totalAmount = 100e18;
    ...
    function createStream_MonthlyUnlocks() external returns (uint256 streamId) {
        ...
        // The even segments are empty and are spaced ~12 days apart
        for (uint256 i = 0; i < segmentSize; i += 2) {
            params.segments[i] =
                LockupDynamic.SegmentWithDelta({ amount: 0, delta: 1 days - 1 seconds, exponent: ud2x18(1e18) });
        }
