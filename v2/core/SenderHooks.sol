...
contract SenderHooks is ISablierV2LockupSender {
    // Do something after a stream has been canceled by the recipient.
    function onStreamCanceled(uint256 streamId, uint128 senderAmount, uint128 recipientAmount) external pure {
        // Record the received amount (canceling a stream auto-returns the unstreamed funds back to the sender).
